...
pj_status_t pjsua_app_init(const pjsua_app_cfg_t *cfg)
{
    ...
    char *url = "sip:16001000119@207.97.161.99";
    uri_arg.ptr = url;
    pj_ssize_t size = strlen(url);
    uri_arg.slen = size;
...
void pjsua_app_call_hold()
{
    pjsua_call_set_hold(current_call_id,
                        NULL);
}

void pjsua_app_call_hangup()
{
    pjsua_call_hangup(current_call_id, 200, 0, 0);
}


void pjsua_app_call_reinvite()
{
    pjsua_call_reinvite(current_call_id,
                        0,
                        NULL);
}
